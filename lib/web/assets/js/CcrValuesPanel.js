...
CcrValuesPanel.prototype.update = function(environment, application, scope, setting, onSuccess, onError) {
    var self = this;
    var uri = buildPath(environment, application, scope, setting);
    ...
    this.serviceClient.post(
        ...
        function(result) {
            result.displayMessage = "Setting was updated successfully.";
            onSuccess(result);
        },
        function(errorDocument) {
            var json = JSON.parse(errorDocument.responseText);
            var message = json.message;
            var issues = json.issues;
            for(var index = 0; index < issues.length; index++) {
                message += '<br/>' + issues[index].name + ' => ' + issues[index].problem;
            }

            errorDocument.displayMessage = message;
