            ...
            if(err != null) {
                ...
                files.forEach(
                    function(file) {
                        ...
                        var instance = new module[cls](datastore, log_writer);
                        instance.configureServer(rest_server);
                        implementations.push(instance);
                    }
                );

                // call start() on the RestifyServer instance.
                log_writer.writeInfo('Starting RestifyServer...');
                rest_server.start();

                log_writer.writeInfo('RestifyServer started. Endpoints are now available.');
            }
        }
    );
